...
import { AppBootstrap, getKycStatusLabel, maskEmailAddress } from '../lib/bootstrap';
...
export function ProfilePage({ bootstrap, onBack, onViewWebsite }: ProfilePageProps) {
  const joinedLabel = bootstrap.user.joinedAt
    ? new Intl.DateTimeFormat('zh-CN', { year: 'numeric', month: 'long' }).format(new Date(bootstrap.user.joinedAt))
    : '刚刚加入';
    ...
    <div className="h-full overflow-y-auto bg-background pb-6">
        ...
        <div className="flex items-start gap-4">
          <div className="relative">
            <div className="w-16 h-16 rounded-full bg-neutral-200 flex items-center justify-center overflow-hidden">
              <span className="text-foreground font-semibold text-xl">{bootstrap.user.initials}</span>
            </div>
            {/* Country flag badge */}
          ...
          <div className="flex-1">
            <h2 className="text-lg font-semibold mb-2">{maskEmailAddress(bootstrap.user.email)}</h2>
            <div className="flex items-center gap-2 mb-2">
              <span className="text-sm text-foreground/60">UID: {bootstrap.user.id}</span>
            ...
            <div className="inline-block bg-neutral-200 text-foreground px-3 py-1 rounded-md text-xs font-medium">
              {getKycStatusLabel(bootstrap.user.kycStatus)}
            </div>
            <div className="mt-2 text-xs text-foreground/45">
              {bootstrap.user.mobileNumber || '未绑定手机号'}
            </div>
          </div>
        ...
        <div className="bg-neutral-100/80 rounded-[24px] p-5 flex items-start gap-4">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-sm text-foreground/60 leading-relaxed">
              邀请伙伴加入 DeePay，可解锁更高等级权益与后续活动奖励。
            </p>
          </div>
      ...
      <div className="px-5 space-y-1">
        ...
        >
          ...
          <div className="flex items-center gap-3">
            <span className="text-sm text-foreground/40">加入于 {joinedLabel}</span>
            <ChevronRight className="w-5 h-5 text-foreground/40" strokeWidth={2.5} />
          </div>
        </motion.button>
      </div>
