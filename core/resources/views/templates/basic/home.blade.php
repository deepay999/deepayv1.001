...
<head>
  ...
  @if($seoContents)
  @if(!empty($seoContents->title))
  <title>{{ $seoContents->title }}</title>
  <meta property="og:title" content="{{ $seoContents->social_title ?? $seoContents->title }}" />
  <meta name="twitter:title" content="{{ $seoContents->social_title ?? $seoContents->title }}" />
  @endif
  @if(!empty($seoContents->description))
  ...
  @php $seoImg = getImage(getFilePath('seo') . '/' . $seoContents->image, getFileSize('seo')); @endphp
