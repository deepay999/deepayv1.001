...
class AutomaticGatewayController extends Controller
{
    ...
    {
        $gateway = Gateway::automatic()->where('alias', 'Swan')->first();
    ...
    public function swanCreate()
    {
        $gateway = Gateway::where('alias', 'Swan')->first();
        ...
        if (!$gateway) {
            ...
            $gateway->gateway_parameters = json_encode([
                ...
                'client_secret' => ['title' => 'Client Secret', 'global' => true, 'value' => ''],
                'merchant_profile_id' => ['title' => 'Merchant Profile ID', 'global' => true, 'value' => ''],
