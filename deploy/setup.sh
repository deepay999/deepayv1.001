...
EOF
  then
    ...
    warn "Could not write PHP-FPM pool config — check permissions on $POOL_CONF_DIR (continuing)."
  fi
else
  warn "Skipped writing PHP-FPM pool config (pool.d directory not found — continuing)."
fi
...
for svc in "php${PHP_MAJOR_MINOR}" "php${PHP_MAJOR_MINOR}-fpm" "php${PHP_VERSION}-fpm" "php-fpm83" "php-fpm84" "php-fpm"; do
  if systemctl is-active --quiet "$svc" 2>/dev/null; then
    systemctl reload "$svc" 2>/dev/null && ok "Reloaded $svc" && break || true
  elif systemctl is-enabled --quiet "$svc" 2>/dev/null; then
    systemctl restart "$svc" 2>/dev/null && ok "Restarted $svc" && break || true
  fi
done || true
