...
cd "$CORE"
"$PHP_BIN" artisan config:cache 2>/dev/null && ok "Config cached."  || warn "config:cache failed"
"$PHP_BIN" artisan route:cache  2>/dev/null && ok "Routes cached."  || warn "route:cache failed"
"$PHP_BIN" artisan view:cache   2>/dev/null && ok "Views cached."   || warn "view:cache failed"
