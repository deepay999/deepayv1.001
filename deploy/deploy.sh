...
if [[ "$SKIP_MIGRATE" == "false" ]]; then
  ...
  "$PHP_BIN" "${CORE_PATH}/artisan" migrate --force --no-interaction \
...
"$PHP_BIN" "${CORE_PATH}/artisan" storage:link --force 2>/dev/null \
...
if [[ "$SKIP_CACHE" == "false" ]]; then
  ...
  "$PHP_BIN" "${CORE_PATH}/artisan" config:clear   2>/dev/null || warn "config:clear failed"
  "$PHP_BIN" "${CORE_PATH}/artisan" route:clear    2>/dev/null || warn "route:clear failed"
  "$PHP_BIN" "${CORE_PATH}/artisan" view:clear     2>/dev/null || warn "view:clear failed"
  "$PHP_BIN" "${CORE_PATH}/artisan" event:clear    2>/dev/null || warn "event:clear failed"
  "$PHP_BIN" "${CORE_PATH}/artisan" config:cache   2>/dev/null || warn "config:cache failed"
  "$PHP_BIN" "${CORE_PATH}/artisan" route:cache    2>/dev/null || warn "route:cache failed"
  "$PHP_BIN" "${CORE_PATH}/artisan" view:cache     2>/dev/null || warn "view:cache failed"
